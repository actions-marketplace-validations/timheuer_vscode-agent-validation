...
const parseAgentFile = (
    filePath: string
): { success: boolean; data?: Record<string, unknown>; body?: string; error?: string } => {
    try {
...
if (fs.existsSync(validFixturesDir)) {
    const validFiles = fs.readdirSync(validFixturesDir).filter((f) => f.endsWith(".agent.md"));
...
tests.push(() => {
    const filePath = path.join(__dirname, "fixtures", "invalid", "no-frontmatter.agent.md");
    const result = parseAgentFile(filePath);
...
tests.push(() => {
    const filePath = path.join(__dirname, "fixtures", "invalid", "invalid-types.agent.md");
    const result = parseAgentFile(filePath);
...
tests.push(() => {
    const filePath = path.join(__dirname, "fixtures", "invalid", "invalid-handoffs.agent.md");
    const result = parseAgentFile(filePath);
